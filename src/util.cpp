...
const char * const BITCOIN_CONF_FILENAME = "socialkredits.conf";
const char * const BITCOIN_PID_FILENAME = "socialkreditsd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "socialkredits" is a composite category enabling all socialkredits-related debug output
            if(ptrCategory->count(string("socialkredits"))) {
...
#else
    const char* pszModule = "socialkredits";
#endif
    if (pex)
        return strprintf(
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\SocialKreditsCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\SocialKreditsCore
    // Mac: ~/Library/Application Support/SocialKreditsCore
    // Unix: ~/.socialkreditscore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "SocialKreditsCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/SocialKreditsCore";
#else
    // Unix
    return pathRet / ".socialkreditscore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty socialkredits.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override socialkredits.conf
